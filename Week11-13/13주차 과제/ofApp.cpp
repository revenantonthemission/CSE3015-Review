...
bool ofApp::BFS()
{
    vector<bool> visit(adjlist.size());
    vector<int>::iterator iter;
    Queue q;
    int cur;
	int point = 0;
	...
	isBFS = 1;
	isDFS = 0;
	distance.assign(adjlist.size(), -2);
    ...
    distance[point] = 0;
    visit[point] = true;
	visited[point / WIDTH][point % WIDTH] = 1;
    parent[point] = point;
    q.push_back(point);
	search_route.push_back(point);
    ...
    while(!q.empty())
    {
        ...
        //cout << endl;
		if(q.get_back() == target) {
            break;
		}
		q.pop_front();
    }
    if(q.get_back() == target) { // 탈출 경로를 역순으로 출력
        cur = target;
        do {
			escape_route.push_back(cur);
            cur = parent[cur];
...
void ofApp::bfsdraw() {
	...
	ofSetColor(ofColor::red);
	for (iter = search_route.begin(); iter != search_route.end(); iter++) {
		i = ((*iter) / WIDTH) * 2 + 1;
		j = ((*iter) % WIDTH) * 2 + 1;
		p = (parent[*iter] / WIDTH) * 2 + 1;
		q = (parent[*iter] % WIDTH) * 2 + 1;
		ofDrawLine(20 * j, 20 * i, 20 * q, 20 * p);
	}
	ofSetColor(ofColor::green);
	for (iter = escape_route.begin(); iter != escape_route.end() - 1; iter++) {
